...
func Encode(level ErrorCorrectionLevel, content string) (*qrcode, error) {
	data := []byte(content)
	vi := findSmallestVersionInfo(level, byteMode, len(data)*8)
	if vi == nil {
		return nil, errors.New("too much data to encode")
	}

	// It's not correct to add the unicode bytes to the result directly but most
	// readers can't handle the required ECI header.
	bits := new(utils.BitList)
	bits.AddBits(int(byteMode), 4)
	bits.AddBits(len(content), vi.charCountBits(byteMode))
	for _, b := range data {
		bits.AddByte(b)
	}
	addPaddingAndTerminator(bits, vi)

	blocks := splitToBlocks(bits.IterateBytes(), vi)
	result := render(blocks.interleave(vi), vi)
	result.content = content
	return result, nil
}

func newQR(dim int) *qrcode {
	return &qrcode{
		dimension: dim,
		rect:      image.Rect(0, 0, dim, dim),
		data:      utils.NewBitList(dim * dim),
	}
}
...
func drawFormatInfo(vi *versionInfo, usedMask int, set func(int, int, bool)) {
	...
	if usedMask == -1 {
		...
		formatInfo = []bool{true, true, true, true, true, true, true, true, true, true, true, true, true, true, true}
	} else {
